...
class GraderTask(EvalTask):
    ...
    def __iter__(self) -> Iterator[EvalSample]:
        for ex in self.dataset.get_test_samples():
            ...
            yield EvalSample(messages=messages, target=target, description=description, difficulty=ex.get('difficulty'), domain=ex.get('domain'))
    ...
    def generate_grader_prompt(self, answer: str, all_key_points: List[str]) -> str:
        instruction = "Your task is to decide which of key points are present in the answer provided and return the letter of the key points that are present in the answer. Here are the key points:\n" +"\n".join([f"{chr(65+i)}. {kp}" for i, kp in enumerate(all_key_points)])
    ...
    def grade(self, init_query: EvalSample, generated_answer: str, grader_model: BaseModel) -> str:
        prompt = self._grading_template.replace("{{QUESTION}}", init_query.messages[0].content).replace("{{KEY_POINTS}}", init_query.target[0]).replace("{{OUTPUT}}", generated_answer.content)
    ...
    def jaccard_distance(self, guess_answer: List[str], true_answer: List[str]) -> float:
