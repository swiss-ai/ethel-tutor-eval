...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", required=True, help="The dataset to use for evaluation: TutorEval"
    )
    parser.add_argument(
        "--model",
        required=True,
        help="The model to use for evaluation: Ethel, Ollama, Smol",
    )
    parser.add_argument(
        "--model_name", required=False, help="The tutor model name to use for model API"
    )
    parser.add_argument(
        "--grader_model",
        required=True,
    ...
    datasets = {"GSM8K": GSM8K, "MATH": MATH, "TutorEval": TutorEval}
    ...
    try:
        ...
        raise ValueError(
            f"Invalid grader model: {args.grader_model}. Supported models: {list(models.keys())}"
        )
    ...
    recorder = Recorder(
        ...
        model_name=(
            f"{args.model}/{args.model_name}" if args.model_name else f"{args.model}"
        ),
    )
    ...
    print(
        f"Evaluated model {args.model} on dataset {args.dataset} with {i} examples and {args.grader_model} grader model:"
    )
